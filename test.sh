...
var0=$(whiptail --title "New Install or Update" --menu \
"Choose new install or update install" ${r} ${c} 2 \
"NEW" "New Install" \
"Update" "Update an existing install" 3>&1 1>&2 2>&3) 

if [ $var0 = NEW ]; then
#install all of the things  
echo "all" > var.conf
#standard install headers
whiptail --msgbox --title "ZeroCar automated installer" "\n\nFirst things first... Lets set up some variables!" ${r} ${c}
#Naming the DLNA Server
var1=$(whiptail --inputbox "Name the DLNA Server" ${r} ${c} ZeroCar --title "DLNA Name" 3>&1 1>&2 2>&3)
#how many WiFi adapters are you running
...
"Two" "Two Adapters" OFF 3>&1 1>&2 2>&3)
  #if one - standard hotspot // if two - then one hotspot and then the other can connect to your home router for ssh/update/upload access
  if [ $var9 = Two ]; then
...
#this is if you selected  Update in the menu at the start 
var6=$(whiptail --title "Lets update your Install" --checklist "select what you want to update by hitting the space bar" ${r} ${c} 7 \
"Distro" "Update the Distro" ON \
"WiFi" "Install WiFi Drivers" OFF \
"Samba" "Update/Restore samba" OFF \
"MiniDLNA" "Update/Restore MiniDLNA" OFF \
"Hostapd" "Update/Restore Hostapd" OFF \
"dnsmasq" "Update/Restore dnsmasq" OFF \
"Droppy" "Update/Restore NPM & Droppy" OFF 3>&1 1>&2 2>&3)
#add the output to a conf file to use grep on later for install variables
echo $var6 > var.conf
#start the update script
$SUDO ./ZeroCarUpdate.sh
fi
...
function install_wifi() { 
  # installing wifi drivers -- Thanks to MrEngman for making the wifi installer!! more info: https://www.raspberrypi.org/forums/viewtopic.php?p=462982
  echo ":::"
...
function install_the_things() {
  ...
  echo "::: DONE!"
  # update Node.js, NPM and install Droppy to allow for web file serving -- thanks to silverwind who made Droppy...makes managing the files much easier thru the web : https://github.com/silverwind/droppy
  echo ":::"
...
function edit_samba() { 
  ...
  #this will ask you for a password for the SAMBA share 
  $SUDO smbpasswd -a pi
